...
def fill_blank_preds_from_imgs(imgs_folder, preds_folder):
    l1 = os.listdir(imgs_folder)
    l2 = os.listdir(preds_folder)
    
    for i in range(len(l1)):
        
        l1[i] = l1[i].replace('jpg','txt')
...
def divide_detections(labels_folder, preds_folder, iou_thresh=0.5):
    ...
    for filename in os.listdir(labels_folder):
        ...
        if labels_tensor.numel()>0: #IF THERE IS A GT LABEL
        ...
        else: #THERE IS NO GT LABEL
            
            #FALSE POSITIVES
            for i in range(preds_tensor.size(dim=0)):
                if preds_tensor[i,0] != -1:
                    FP.append(np.array([preds_tensor[i,5]]))
